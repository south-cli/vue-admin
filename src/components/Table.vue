...
import { VXETable, VxeGridInstance, VxeGridProps, VxeTableDefines } from 'vxe-table'
import XEUtils from 'xe-utils'

const columnList: unknown[] = []
...
export default defineComponent({
  setup () {
    ...
    const mockColumns = (colSize: number): Promise<unknown[]> => {
    ...
    const mockList = (rowSize: number): Promise<unknown[]> => {
      return new Promise(resolve => {
        setTimeout(() => {
    ...
    const loadColumnAndData = (colSize: number, rowSize: number) => {
      ...
      Promise.all([
        ...
        if ($grid) {
          Promise.all([
            $grid.reloadColumn(columns as VxeTableDefines.ColumnOptions[]),
